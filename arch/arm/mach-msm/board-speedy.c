...
{
	int rc;
	static struct pm_gpio kypd_sns = {
	...
	static struct pm_gpio p_sensor = {
		.direction      = PM_GPIO_DIR_IN,
		.output_buffer  = 0,
		.output_value   = 0,
	...
	static struct pm_gpio ls_sensor = {
		.direction      = PM_GPIO_DIR_OUT,
		.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
		.output_value   = 0,
		.pull           = PM_GPIO_PULL_NO,
	...
	/* Qty keypad SNS */
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_KYPD_SNS1), &kypd_sns);
	if (rc)
		pr_err("%s SPEEDY_KYPD_SNS1 write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_KYPD_SNS2), &kypd_sns);
	if (rc)
		pr_err("%s SPEEDY_KYPD_SNS2 write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_KYPD_SNS3), &kypd_sns);
	if (rc)
		pr_err("%s SPEEDY_KYPD_SNS3 write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_KYPD_SNS4), &kypd_sns);
	if (rc)
		pr_err("%s SPEEDY_KYPD_SNS4 write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_KYPD_SNS5), &kypd_sns);
	if (rc)
		pr_err("%s SPEEDY_KYPD_SNS5 write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_KYPD_SNS6), &kypd_sns);
	if (rc)
		pr_err("%s SPEEDY_KYPD_SNS6 write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_KYPD_SNS7), &kypd_sns);
	if (rc)
		pr_err("%s SPEEDY_KYPD_SNS7 write failed\n", __func__);

	/* led */
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GREEN_LED), &led);
	if (rc)
		pr_err("%s SPEEDY_GREEN_LED write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_AMBER_LED), &led);
	if (rc)
		pr_err("%s SPEEDY_GREEN_LED write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_KEYPAD_LED), &led);
	if (rc)
		pr_err("%s SPEEDY_GREEN_LED write failed\n", __func__);

	/* direct key */
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_VOL_UP), &volume);
	if (rc)
		pr_err("%s SPEEDY_VOL_UP write failed\n", __func__);

	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_VOL_DN), &volume);
	if (rc)
		pr_err("%s SPEEDY_VOL_UP write failed\n", __func__);


	/* touch panel */
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_TP_RSTz), &tp_rstz);
	if (rc)
		pr_err("%s SPEEDY_TP_RSTz write failed\n", __func__);

	/* audio power */
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_AUD_SPK_ENO), &audio);
	if (rc)
		pr_err("%s SPEEDY_AUD_SPK_ENO write failed\n", __func__);
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_AUD_HANDSET_ENO), &audio);
	if (rc)
		pr_err("%s SPEEDY_AUD_HANDSET_ENO write failed\n", __func__);

	/* P-sensor and light sensor */
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_PS_EN), &p_sensor);
	if (rc)
		pr_err("%s SPEEDY_GPIO_PS_EN write failed\n", __func__);

	if (system_rev == 0) {
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_PS_EN), &ls_sensor);
		if (rc)
			pr_err("%s SPEEDY_GPIO_PS_EN write failed\n", __func__);
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_LS_EN), &ls_sensor);
		if (rc)
			pr_err("%s SPEEDY_GPIO_LS_EN write failed\n", __func__);
	}

	/* wimax wakeup host source */
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_WiMAX_HOST_WAKEUP), &wimax);
	if (rc)
		pr_err("%s SPEEDY_WiMAX_HOST_WAKEUP write failed\n", __func__);

	if (system_rev >= 1) {
		/* G Sensor INT*/
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_GSENSOR_INT_N_XB), &sensors);
		if (rc)
			pr_err("%s SPEEDY_GPIO_GSENSOR_INT_N_XB write failed\n", __func__);
		/* E-Compass INT */
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_COMPASS_INT_N_XB), &sensors);
		if (rc)
			pr_err("%s SPEEDY_GPIO_COMPASS_INT_N_XB write failed\n", __func__);

		/* uP Reset */
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_uP_RST_XB), &microp);
		if (rc)
			pr_err("%s SPEEDY_GPIO_uP_RST_XB write failed\n", __func__);
	}
...
static struct android_usb_platform_data android_usb_pdata = {
	...
	.nluns = 1,
	.usb_id_pin_gpio = SPEEDY_GPIO_USB_ID1_PIN,
};
...
static struct platform_device speedy_flashlight_device = {
...
static uint32_t usb_ID_PIN_input_table[] = {
	GPIO_CFG(SPEEDY_GPIO_USB_ID1_PIN, 0, GPIO_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};

static uint32_t usb_ID_PIN_ouput_table[] = {
	GPIO_CFG(SPEEDY_GPIO_USB_ID1_PIN, 0, GPIO_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
...
void config_speedy_usb_id_gpios(bool output)
{
	if (output) {
		...
		gpio_set_value(SPEEDY_GPIO_USB_ID1_PIN, 1);
		printk(KERN_INFO "%s %d output high\n",  __func__, SPEEDY_GPIO_USB_ID1_PIN);
	} else {
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.dock_pin_gpio  = SPEEDY_GPIO_DOCK_PIN,
};
...
/* HTC_WIFI_START */
/*
static unsigned int speedy_sdc3_slot_type = MMC_TYPE_SDIO_WIFI;
static struct mmc_platform_data msm7x30_sdc3_data = {
	...
	.msmsdcc_fmid	= 24576000,
	.msmsdcc_fmax	= 49152000,
	.slot_type		= &speedy_sdc3_slot_type,
	.nonremovable	= 0,
};
*/
/* HTC_WIFI_END */
...
static struct mmc_platform_data msm7x30_sdc4_data = {
	...
	.status_irq  = PM8058_GPIO_IRQ(PMIC8058_IRQ_BASE, SPEEDY_SDMC_CD_N),
...
static int speedy_ts_power(int on)
{
	...
	if (on) {
		...
		msleep(5);
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_TP_EN), 1);
		msleep(5);
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(SPEEDY_TP_RSTz), 1);
		msleep(40);
	} else {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_TP_EN), 0);
		msleep(2);
	}
...
static ssize_t speedy_atmel_virtual_keys_show(struct kobject *kobj,
...
static struct kobj_attribute speedy_atmel_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.atmel-touchscreen",
		.mode = S_IRUGO,
	},
	.show = &speedy_atmel_virtual_keys_show,
};

static struct attribute *speedy_properties_attrs[] = {
	&speedy_atmel_virtual_keys_attr.attr,
	NULL
};
...
static int config_speedy_proximity_gpios(int on)
{
	...
	static struct pm_gpio proximity_on = {
		.direction      = PM_GPIO_DIR_IN,
		.output_buffer  = 0,
		.output_value   = 0,
		.pull           = PM_GPIO_PULL_UP_31P5,
	...
	static struct pm_gpio proximity_off = {
		.direction      = PM_GPIO_DIR_IN,
		.output_buffer  = 0,
		.output_value   = 0,
		.pull           = PM_GPIO_PULL_DN,
	...
	if (on) {
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_PS_EN), &proximity_on);
		if (rc)
			pr_err("%s PMIC GPIO P-sensor interrupt write failed\n", __func__);
	} else {
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SPEEDY_GPIO_PS_EN), &proximity_off);
		if (rc)
			pr_err("%s PMIC GPIO P-sensor interrupt write failed\n", __func__);
	}
...
static struct i2c_board_info i2c_microp_devices_XA[] = {
	{
...
static struct i2c_board_info i2c_microp_devices_XB[] = {
	{
		I2C_BOARD_INFO(MICROP_I2C_NAME, 0xCC >> 1),
...
static struct i2c_board_info i2c_Sensors_devices_XA[] = {
	{
		I2C_BOARD_INFO(BMA150_I2C_NAME, 0x70 >> 1),
		.platform_data = &gsensor_platform_data_XA,
		.irq = SPEEDY_GPIO_GSENSOR_INT_N_XA,
	},
	{
		I2C_BOARD_INFO(AKM8975_I2C_NAME, 0x1A >> 1),
		.platform_data = &compass_platform_data,
		.irq = PM8058_MPP_IRQ(PMIC8058_IRQ_BASE, 0)+1,
	},
};

static struct i2c_board_info i2c_Sensors_devices_XB[] = {
	{
		I2C_BOARD_INFO(BMA150_I2C_NAME, 0x70 >> 1),
...
static void __init speedy_init(void)
{
	...
	if (system_rev >= 1) {
		i2c_register_board_info(0, i2c_Sensors_devices_XB,
			ARRAY_SIZE(i2c_Sensors_devices_XB));
		i2c_register_board_info(0, i2c_microp_devices_XB,
			ARRAY_SIZE(i2c_microp_devices_XB));
	} else {
		i2c_register_board_info(0, i2c_Sensors_devices_XA,
			ARRAY_SIZE(i2c_Sensors_devices_XA));
		i2c_register_board_info(0, i2c_microp_devices_XA,
			ARRAY_SIZE(i2c_microp_devices_XA));
