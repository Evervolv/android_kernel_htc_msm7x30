...
)
{
	...
	if (iwpmksa->cmd == IW_PMKSA_FLUSH) {
		...
		for (i = 0; i < pmkid_list.pmkids.npmkid; i++)
			if (!bcmp(&iwpmksa->bssid.sa_data[0], &pmkid_list.pmkids.foo[i].BSSID,
				ETHER_ADDR_LEN))
				break;

		if ((pmkid_list.pmkids.npmkid > 0) && (i < pmkid_list.pmkids.npmkid)) {
			bzero(&pmkid_list.pmkids.foo[i], sizeof(pmkid_t));
			for (; i < (pmkid_list.pmkids.npmkid - 1); i++) {
				bcopy(&pmkid_list.pmkids.foo[i+1].BSSID,
					&pmkid_list.pmkids.foo[i].BSSID,
					ETHER_ADDR_LEN);
				bcopy(&pmkid_list.pmkids.foo[i+1].PMKID,
					&pmkid_list.pmkids.foo[i].PMKID,
					WPA2_PMKID_LEN);
			}
		...
		if (i < MAXPMKID) {
			bcopy(&iwpmksa->bssid.sa_data[0],
				&pmkid_list.pmkids.foo[i].BSSID,
				ETHER_ADDR_LEN);
			bcopy(&iwpmksa->pmkid[0], &pmkid_list.pmkids.foo[i].PMKID,
				WPA2_PMKID_LEN);
	...
	for (i = 0; i < pmkid_list.pmkids.npmkid; i++) {
		uint j;
		WL_WSEC(("PMKID[%d]: %s = ", i,
			bcm_ether_ntoa(&pmkid_list.pmkids.foo[i].BSSID,
			eabuf)));
		for (j = 0; j < WPA2_PMKID_LEN; j++)
			WL_WSEC(("%02x ", pmkid_list.pmkids.foo[i].PMKID[j]));
		WL_WSEC(("\n"));
	}
	WL_WSEC(("\n"));
